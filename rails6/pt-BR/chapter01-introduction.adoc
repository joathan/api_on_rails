...
Após modificar o arquivo _.gitignore_, precisamos apenas adicionar os arquivos e _commitir_ as alterações, para isso usamos os seguintes comandos:
...
TIP: Descobri que a mensagem de confirmação deve começar com um verbo no tempo presente, descrevendo o que a confirmação faz e não o que fez; isso ajuda quando você está explorando o histórico do projeto. Achei isso mais natural de ler e entender. Continuaremos esta prática até o final do tutorial.
...
Em seguida, executamos o código _push_:
